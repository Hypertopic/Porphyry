...
class Outliner extends React.Component {
      ...
      <div className="App container-fluid">
        ...
        <div className="Status row h5">
          ...
          <Link to="/" className="badge badge-pill badge-light TopicTag">
            <span className="badge badge-pill badge-dark oi oi-chevron-left" />
            Retour à l'accueil
          </Link>
        </div>
                  ...
                  {this.state.title ? (
                    <ul className="Outliner">
                      <Node
                        ...
                        draggedTopic={this.state.draggedTopic}
                        change={this.editTopic.bind(this)}
                        activate={this.activeNode.bind(this)}
                        id="root"
                      />
                    </ul>
                  ) : (
    ...
    return (
      <form className="input-group" onSubmit={e => this._newVP(e)}>
        <input
          ...
          placeholder="Nom du point de vue"
        />
        <div className="input-group-append">
          <button type="submit" className="btn add btn-sm btn-light">
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        if (!e.altKey && !e.ctrlKey && !e.shiftKey) {
          this.activeNode(
            this.topicTree.getPreviousTopic(this.state.activeNode)
          );
        }
        break;
      case 'ArrowDown':
      ...
      case 'Backspace':
        if (!e.altKey && !e.ctrlKey && !e.shiftKey) {
          if (e.target.tagName === 'BODY' || e.target.value === '') {
            let previousTopic = this.topicTree.getPreviousTopic(
              this.state.activeNode
            );
            changed = this.topicTree.deleteTopic(this.state.activeNode);
            if (changed)
              this.activeNode(this.topicTree.getNextTopic(previousTopic));
          }
        }
        break;
      default:
              ...
              switch (key) {
                case 'parent':
                  if (this.topicTree.getTopic(change.parent)) {
                    toApply =
                      this.topicTree.setParent(id, change.parent) &&
  ...
  _fetchData() {
    if (!this.changing) {
      return db.get({ _id: this.props.match.params.id }).then(x => {
...
class Node extends React.Component {
  ...
  render = () => {
    ...
    if (this.state.edit) {
      ...
      thisNode = (
        <span className="node" onDoubleClick={switchEdit}>
          {this.props.me.name}
        </span>
      );
    }
    ...
    if (this.props.topics) {
      for (var topID in this.props.topics) {
        ...
        if (
          (this.props.id && topic.broader.indexOf(this.props.id) !== -1) ||
          ...
          children.push(
            <Node
              ...
              draggedTopic={this.props.draggedTopic}
    ...
    let onDrag = e => {
      var draggedTopic =
        e.dataTransfer.getData('dragContent') || this.props.draggedTopic;
      if (!draggedTopic) {
      ...
      if (
        draggedTopic === this.props.id ||
        topicTree.isAncestor(draggedTopic, this.props.id)
      ) {
        //can't be dropped into itself or its descendant
      } else if (e.currentTarget.className === 'wrap') {
        //child
    ...
    let onDrop = e => {
      ...
      if (
        droppedTopic === this.props.id ||
        topicTree.isAncestor(droppedTopic, this.props.id)
      ) {
        //can't be dropped into itself or its descendant
      } else if (e.currentTarget.className === 'wrap') {
        //child
    ...
    return (
      <li
        ...
        onDragEnd={onDragStop.bind(this)}
      >
        <span
          className="caret"
          onClick={switchOpen}
        ...
        <span
          ...
          onDragLeave={onDragLeave}
          onClick={activeMe}
          onDoubleClick={setEdit.bind(this)}
        >
          {thisNode}
